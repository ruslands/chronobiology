...
class Chronobiology:
    ...
    def filter_inactive(self, min_data_points=1):
        ...
        self.__mask[:] = False
        events, _ = np.histogram(
            self.__timestamps[self.__activity > 0],
    ...
    def plot_actogram(
        ...
        fig, subplots = plt.subplots(
            ...
            sharex=True,
            sharey=True,
            figsize=(width / dpi, total_height / dpi),
            dpi=dpi,
            gridspec_kw={
                "hspace": 0,
                "wspace": 0,
        ...
        for i in range(1, 2 * self.days):
            ...
            ax.set_yticks([])
            ax.set_ylabel(
                f"{pd.Timestamp(timestamps[d*steps_per_day]).strftime('%Y-%m-%d')}",
                rotation=0,
                fontsize=8,
                labelpad=40,
                va="center",
    ...
    def plot_periodogram(
        ...
        fig.text(
            0.5,
            0.0,
            f"Maximal power at period " f"{periods[values.argmax()] / hour :.2f} hours",
            ha="center",
            fontsize=20,
            wrap=False,
        )
    ...
    def plot_light_activity(self, bouts=False, filename=None, width=1000, height=600, dpi=100):
        ...
        fig.text(
            0.5,
            0.95,
            f"Light Phase Activity {self.descr}",
            ha="center",
            fontsize=20,
            wrap=False,
        )
    ...
    def plot_intradaily_variability(self, step="1h", bouts=False, filename=None, width=1000, height=600, dpi=100):
        ...
        fig.text(
            0.5,
            0.95,
            f"Intradaily Variability {self.descr}",
            ha="center",
            fontsize=20,
            wrap=False,
        )
    ...
    def plot_relative_amplitude(
        ...
        fig.text(
            0.5,
            0.95,
            f"Relative Amplitude {self.descr}",
            ha="center",
            fontsize=20,
            wrap=False,
        )
    ...
    def daily_bouts(self, max_gap=None, min_duration=None, min_activity=None):
        ...
        """
        bouts = (
            self.activity_bouts(max_gap, min_duration, min_activity)
            .reshape(self.days, self.steps_per_day)
            .astype("int")
        )
    ...
    def plot_bout_histogram(
        ...
        fig.text(
            0.5,
            0.95,
            f"Bout length distribution {self.descr}",
            ha="center",
            fontsize=20,
            wrap=False,
        )
