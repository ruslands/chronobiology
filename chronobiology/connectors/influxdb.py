...
class InfluxDBConnector:
    ...
    def get_fields(self, series, return_types=False):
        ...
        if result:
            if return_types:
                return (
                    [x[0] for x in result[0]["values"]],
                    [x[1] for x in result[0]["values"]],
                )
            else:
                return [x[0] for x in result[0]["values"]]
        else:
    ...
    def get_data(
        self, series, fields, keys=None, start=None, stop=None, local_tz=False
    ):
        ...
        if type(fields) is dict:
            ...
            raise TypeError(
                f"fields should be a string, list, tuple, set or dict but {type(fields)} was passed"
            )
        ...
        if keys is None or keys == {}:
            ...
            raise ValueError(
                f"keys should be None or dic of key: value pairs but {type(keys)} was passed"
            )
        else:
            where_clause = (
                f" WHERE {' AND '.join([_destructure(k, v) for k, v in keys.items()])}"
            )
        time_query = ""
        ...
        query = (
            f'SELECT {", ".join(qfields)} FROM "{series}"{where_clause}{time_query};'
        )
